      ...
      <KeyboardAvoidingView
        ...
        <ScrollView
          ...
          <View className="rounded-card bg-surface p-5 shadow-sm">
            <Text className="text-center text-xl font-bold text-text">{s.completeProfileTitle}</Text>
            <Text className="mt-2 text-center text-sm leading-5 text-text-secondary">
              {s.completeProfileSubtitle}
            </Text>

            <View className="mt-6 flex-row items-center gap-2">
              <Ionicons name="business-outline" size={16} color="#666666" />
              <Text className="text-sm text-text-secondary">{s.companyName}</Text>
            </View>
            <TextInput
              value={companyName}
            ...
            <View className="mt-5 flex-row items-center gap-2">
              ...
              <Text className="text-sm text-text-secondary">{s.gstNumber}</Text>
            </View>
            <TextInput
              value={gstNumber}
              onChangeText={setGstNumber}
              placeholder={s.gstPlaceholder}
              placeholderTextColor="#999999"
              autoCapitalize="characters"
              className="mt-2 rounded-input border border-border bg-surface px-4 py-3.5 text-base text-text"
            />

            <Text className="mt-8 text-base font-bold text-text">{s.chooseLanguage}</Text>
            <Text className="mt-1 text-sm text-text-secondary">{s.languageSubtitle}</Text>
            <Text className="mt-2 text-sm font-medium text-primary">{s.hindiLink}</Text>
            ...
            <View className="mt-3 flex-row items-center gap-2 self-start rounded-full bg-timer px-3 py-1.5">
              ...
              <Text className="text-xs font-medium text-secondary">{s.recommendedRegion}</Text>
            </View>
          ...
          <View className="mt-6 flex-row items-center justify-center gap-1.5">
            ...
            <Text className="text-[10px] tracking-wider text-text-secondary">{s.isoSecurity}</Text>
          </View>
        </ScrollView>
